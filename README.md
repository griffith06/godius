...
- Edit translated text values only. Do not change JSON keys.
- Keep placeholders such as `%s`, `%d`, `%u`, `{0}`, and `\n`.
- Keep color tags such as `<green>`, `<orange>`, `<red>`, `<blue>`, and `</green>`.
- Do not remove item names, skill names, NPC names, or map names unless you are correcting an official term.
- If you are unsure, submit feedback through the form instead of editing JSON directly.

## 한국어

가디우스 번역 품질 개선을 위한 공개 제보 저장소입니다.
...
This repository is for improving Godius localization quality.
...
このリポジトリは、Godius の翻訳品質を改善するための公開フィードバック用です。
...
Dieses Repository dient zur Verbesserung der Godius-Lokalisierung.
...
Este repositorio sirve para mejorar la localización de Godius.
...
- Texto sugerido
- Dónde lo viste
- Enlace a captura si está disponible

## Français

Ce dépôt sert à améliorer la localisation de Godius.
...
- Lien de capture d'écran si disponible

## Polski

To repozytorium służy do poprawy lokalizacji Godius.
...
- Proponowany tekst
- Gdzie go zobaczono
- Link do zrzutu ekranu, jeśli jest dostępny

## Português (Brasil)

Este repositório serve para melhorar a localização de Godius.
...
- Texto sugerido
- Onde você viu
- Link de captura de tela, se disponível

## Русский

Этот репозиторий предназначен для улучшения локализации Godius.
