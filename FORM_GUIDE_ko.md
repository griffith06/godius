...
2. 일반 유저는 Form으로 받고, GitHub 사용자는 Pull Request로 받습니다.
3. 매일 1회 이상 Sheets에서 중복 제보를 정리합니다.
4. 언어별로 `Accepted` 또는 `Modified` 상태를 고릅니다.
5. 내부 검수자가 JSON에 반영합니다.
6. 패치노트에 "Translation improvements based on community feedback"로 공지합니다.
